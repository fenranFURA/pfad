...
def game_loop(screen, screen_width, screen_height, paddle_images, pause_image, main_image):
    running = True
    clock = pygame.time.Clock()

    # 初始化游戏状态
    paddle_index = 0
    paddle_image = paddle_images[paddle_index]
    ...
    # 特殊事件标志
    paddle_change_timer = pygame.time.get_ticks()
    paddle_change_interval = random.randint(5000, 10000)  # 初始间隔为5到10秒
    ...
    while running:
            ...
            if event.type == pygame.QUIT:
                ...
                if pause_rect.collidepoint(event.pos):
                    ...
                    if action == "restart":
                        ...
                        bricks = create_bricks()
                        paddle_change_timer = pygame.time.get_ticks()
                        paddle_change_interval = random.randint(5000, 10000)
        ...
        # 球与挡板碰撞检测
        paddle_rect = pygame.Rect(paddle_x, paddle_y, paddle_image.get_width(), paddle_image.get_height())
        ...
        for brick in bricks[:]:
            ...
            if (brick_x - ball_x) ** 2 + (brick_y - ball_y) ** 2 <= (brick_radius + 15) ** 2:
                ...
                if current_time - paddle_change_timer >= paddle_change_interval:
                    paddle_index = random.randint(0, len(paddle_images) - 1)
                    paddle_image = paddle_images[paddle_index]
                    paddle_change_timer = current_time
                    paddle_change_interval = random.randint(5000, 10000)  # 新的间隔为5到10秒
...
def main():
    ...
    # 加载图像
    paddle_images = [
        pygame.image.load("Enlargement.png").convert_alpha(),
        pygame.image.load("reduction.png").convert_alpha(),
        pygame.image.load("path_to_your_paddle_image.png").convert_alpha()
    ]

    game_loop(screen,screen_width,screen_height,paddle_images,pause_image,main_image)
